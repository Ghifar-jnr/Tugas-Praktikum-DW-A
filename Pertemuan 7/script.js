...
function tampilkanDaftarBuku() {
    ...
    daftarBuku.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            ...
            <button class="mt-2.5 mb-2.5 bg-sky-500 px-2 py-1 rounded border-2 border-solid border-black border-sky-700 cursor-pointer hover:bg-sky-600" onclick="tambahKeFavorit(${index})">Tambah ke Favorit</button>
            <button class="mt-2.5 bg-red-500 px-2 py-1 rounded border-2 border-solid border-black border-red-700 cursor-pointer hover:bg-red-600" onclick="hapusDariDaftar(${index})">Hapus</button>
            <p class="mt-2.5 mb-2.5 border-b border-solid border-slate-500"></p>
        `;
...
function tampilkanBukuFavorit() {
    ...
    bukuFavorit.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            <p>${buku.tampilkanInfo()}</p>
            <button class="mt-2.5 bg-red-500 px-2 py-1 rounded border-2 border-solid border-black border-red-700 cursor-pointer hover:bg-red-600" onclick="hapusDariFavorit(${index})">Hapus</button>
            <p class="mt-2.5 mb-2.5 border-b border-solid border-slate-500"></p>
        `;
...
function hapusDariDaftar(index) {
    ...
    if(hapusDariDaftar) {
        alert('Apakah Anda Yakin Ingin Menghapus Dari Daftar Buku ? ');
        return;
    }
}
